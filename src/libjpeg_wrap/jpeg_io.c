    ...
    while (src->bytes_in_buffer <= datacnt)  {
        ...
        if (src->bytes_in_buffer == 0) {
            *n_bytes -= datacnt;
            ...
            /*we should really check if this was preceeded by a 0xFF
            byte, but since we don't support suspension of the data
            stream, having only one byte in the buffer should mean its
            the last byte*/
            ...
            *n_bytes -= (datacnt - src->bytes_in_buffer);
            /****special case for boundary aligned EOI****/
            /*End-of-file is indicated to the hardware routine whenever
            less than n_bytes is returned from this call. If the
            EOI is perfectly aligned on the buffer boundary, the end-of
            -file will not be determined until the next call which will
            ...
            if (datacnt == src->bytes_in_buffer) {
                *(((JOCTET *)dataptr)+datacnt-1) = 0xFF;  //tag on the extra byte
                datacnt--;
            } else {
                datacnt = src->bytes_in_buffer;
            }
